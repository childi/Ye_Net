...
class YOLOV3(object):
    ...
    def __init__(self, input_data, trainable, avod=None, pred2d=True):
    ...
    def __build_nework(self, input_data, avod=None):
        ...
        # self.darknet_fe = common.convolutional(darknet_fe, (1, 1, 1024,  32), self.trainable, 'darknet_fe')
        ...
        conv_lbbox = common.convolutional(conv_lobj_branch, (1, 1, 1024, 3*(self.num_class + 8)),
        ...
        conv_mbbox = common.convolutional(conv_mobj_branch, (1, 1, 512, 3*(self.num_class + 8)),
        ...
        conv_sbbox = common.convolutional(conv_sobj_branch, (1, 1, 256, 3*(self.num_class + 8)),
    ...
    def decode(self, conv_output, anchors, stride):
        ...
        conv_raw_alpha_cos = conv_output[:, :, :, :, 6:7]
        conv_raw_alpha_sin = conv_output[:, :, :, :, 7:8]
        conv_raw_prob = conv_output[:, :, :, :, 8:]
        ...
        return tf.concat([pred_xywh, pred_conf, pred_z, pred_alpha, pred_prob], axis=-1)
    ...
    def loss_layer(self, conv, pred, label, bboxes, anchors, stride):
        ...
        conv = tf.reshape(conv, (batch_size, output_size, 3 * output_size,
                                 self.anchor_per_scale, 8 + self.num_class))
        conv_raw_conf = conv[:, :, :, :, 4:5]
        # conv_raw_alpha_angle = conv[:, :, :, :, 5:7]
        conv_raw_z = conv[:, :, :, :, 5:6]
        conv_raw_alpha_angle = conv[:, :, :, :, 6:8]
        ...
        return giou_loss, conf_loss, prob_loss, z_loss, alpha_loss
    ...
    def compute_loss(self, label_sbbox, label_mbbox, label_lbbox, true_sbbox, true_mbbox, true_lbbox):
        ...
        with tf.name_scope('alpha_loss'):
            alpha_loss = loss_sbbox[4] + loss_mbbox[4] + loss_lbbox[4]
