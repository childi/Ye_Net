...
class YOLOV3(object):
    ...
    def __init__(self, input_data, trainable, p2_I, avod=None, pred2d=True):
        ...
        # try:
        self.conv_lbbox, self.conv_mbbox, self.conv_sbbox, self.conv_lz, self.conv_mz, self.conv_sz = \
            self.__build_nework(input_data, avod=avod)
        # except:
        #     raise NotImplementedError("Can not build up yolov3 network!")

        if pred2d is True:
            with tf.variable_scope('pred_sbbox'):
                self.pred_sbbox = self.decode(self.conv_sbbox, self.conv_sz, self.anchors[0], self.strides[0], p2_I)

            with tf.variable_scope('pred_mbbox'):
                self.pred_mbbox = self.decode(self.conv_mbbox, self.conv_mz, self.anchors[1], self.strides[1], p2_I)

            with tf.variable_scope('pred_lbbox'):
                self.pred_lbbox = self.decode(self.conv_lbbox, self.conv_lz, self.anchors[2], self.strides[2], p2_I)
    ...
    def __build_nework(self, input_data, avod=None):
        ...
        conv_lz_branch = common.convolutional(input_data, (3, 3, 512, 1024), self.trainable, name='conv_lz_branch')
        conv_lz = common.convolutional(conv_lz_branch, (1, 1, 1024, 3 * 1),
                                       trainable=self.trainable, name='conv_lz', activate=False, bn=False)
        ...
        conv_mz_branch = common.convolutional(input_data, (3, 3, 256, 512), self.trainable, name='conv_mz_branch')
        conv_mz = common.convolutional(conv_mz_branch, (1, 1, 512, 3 * 1),
                                       trainable=self.trainable, name='conv_mz', activate=False, bn=False)
        ...
        conv_sz_branch = common.convolutional(input_data, (3, 3, 128, 256), self.trainable, name='conv_sz_branch')
        conv_sz = common.convolutional(conv_sz_branch, (1, 1, 256, 3 * 1),
                                       trainable=self.trainable, name='conv_sz', activate=False, bn=False)
        ...
        return conv_lbbox, conv_mbbox, conv_sbbox, conv_lz, conv_mz, conv_sz

    def decode(self, conv_output, conv_z, anchors, stride, cam_p2_I):
        """
        ...
        conv_raw_z = tf.reshape(conv_z, (batch_size, output_size, 3 * output_size, anchor_per_scale, 1))
    ...
    def bbox_giou(self, boxes1, boxes2):
        ...
        enclose = tf.maximum(enclose_right_down - enclose_left_up, 0.0001)  # 0.0
    ...
    def bbox_diou(self, boxes1, boxes2):
        ...
        boxes1_center = boxes1[..., :2]
        boxes2_center = boxes2[..., :2]
        ...
        inter_area = inter_section[..., 0] * inter_section[..., 1]
        # union_area = boxes1_area + boxes2_area - inter_area
        union_area = tf.maximum(boxes1_area + boxes2_area - inter_area, 0.000001)
        iou = inter_area / union_area
        ...
        # enclose = tf.maximum(enclose_right_down - enclose_left_up, 0.0001)  # 0.0
        # enclose_area = enclose[..., 0] * enclose[..., 1]
        inter_diag = (boxes1_center[..., 0] - boxes2_center[..., 0]) ** 2 + \
                     (boxes1_center[..., 1] - boxes2_center[..., 1]) ** 2
        outer_diag = (enclose_right_down[..., 0] - enclose_left_up[..., 0]) ** 2 + \
                     (enclose_right_down[..., 1] - enclose_left_up[..., 1]) ** 2
        # giou = iou - 1.0 * (enclose_area - union_area) / enclose_area
        diou = iou - (inter_diag) / tf.maximum(outer_diag, 0.000001)
        diou = tf.clip_by_value(diou, -1.0, 1.0)
    ...
    def loss_layer(self, conv, pred, label, bboxes, anchors, stride):
        ...
        bbox_loss_scale = tf.clip_by_value(bbox_loss_scale, 0.0, 2.0)
        ...
        conv_norm_z = (pred_z + 2) / 90
        label_norm_z = (label_z + 2) / 90
        z_loss = respond_bbox * (self.WeightedSmoothL1Loss(label_norm_z, conv_norm_z))

        # offsets_z_gt = label_z - (pred_z - conv_raw_z)
        # z_loss = respond_bbox * (self.WeightedSmoothL1Loss(offsets_z_gt, conv_raw_z))
        ...
        return giou_loss, conf_loss, prob_loss, z_loss
