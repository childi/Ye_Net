...
class YOLOV3(object):
    ...
    def __init__(self, input_data, trainable, p2_I, avod=None, pred2d=True):
        ...
        if pred2d is True:
            with tf.variable_scope('pred_sbbox'):
                self.pred_sbbox = self.decode(self.conv_sbbox, self.anchors[0], self.strides[0], p2_I)

            with tf.variable_scope('pred_mbbox'):
                self.pred_mbbox = self.decode(self.conv_mbbox, self.anchors[1], self.strides[1], p2_I)

            with tf.variable_scope('pred_lbbox'):
                self.pred_lbbox = self.decode(self.conv_lbbox, self.anchors[2], self.strides[2], p2_I)
    ...
    def decode(self, conv_output, anchors, stride, cam_p2_I):
        """
        ...
        pred_x = pred_xywh[:, :, :, :, 0:1]
        pred_y = pred_xywh[:, :, :, :, 1:2]
        pred_w = pred_xywh[:, :, :, :, 2:3]
        pred_h = pred_xywh[:, :, :, :, 3:4]
        center_t_2d = tf.concat([pred_x, pred_y - pred_h / 2, tf.ones_like(pred_x)], axis=-1)
        center_b_2d = tf.concat([pred_x, pred_y + pred_h / 2, tf.ones_like(pred_x)], axis=-1)
        # cam_p2_I = tf.matrix_inverse(cam_p2)
        center_t_3d = tf.matmul(tf.tile(cam_p2_I[:, tf.newaxis, tf.newaxis, tf.newaxis, :, :],
                                        [1, output_size, 3 * output_size, anchor_per_scale, 1, 1])
                                , center_t_2d[:, :, :, :, :, tf.newaxis])
        center_b_3d = tf.matmul(tf.tile(cam_p2_I[:, tf.newaxis, tf.newaxis, tf.newaxis, :, :],
                                        [1, output_size, 3 * output_size, anchor_per_scale, 1, 1])
                                , center_b_2d[:, :, :, :, :, tf.newaxis])
        # similar triangles from default height and 2Dbox_height
        mean_h_3d = 1.5
        d = mean_h_3d / (tf.squeeze(center_b_3d, squeeze_dims=[-1])[:, :, :, :, 1:2] -
                         tf.squeeze(center_t_3d, squeeze_dims=[-1])[:, :, :, :, 1:2])
        pred_z = d + conv_raw_z
