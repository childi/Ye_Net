...
def euclidean_distance(box, centroid):
    coords_w = box.w - centroid.w
    coords_h = box.h - centroid.h
    return np.sqrt(np.sum(coords_w ** 2 + coords_h ** 2))
...
def do_kmeans(n_anchors, boxes, centroids):
    ...
    for box in boxes:
        ...
        for centroid_index, centroid in enumerate(centroids):
            distance = (1 - box_iou(box, centroid))
            # distance = euclidean_distance(box, centroid)
            if distance < min_distance:
                min_distance = distance
...
def compute_centroids(label_path,n_anchors,loss_convergence,grid_size,iterations_num,plus):
    ...
    for label_file in label_files:
        ...
        for line in f:
            ...
            if len(temp) > 1 and temp[0] == str('Car'):
                boxes.append(Box(0, 0, (float(temp[6]) - float(temp[4])), (float(temp[7]) - float(temp[5]))))  # 2d wh
                # boxes.append(Box(0, 0, float(temp[9]), float(temp[10])))  # 3d lw
    ...
    for centroid in centroids:
        wh = [centroid.w, centroid.h]
        center.append(wh)
        print(centroid.w, centroid.h)
    center = np.array(center)

    color = ['magenta', 'orange', 'yellow', 'green', 'cyan', 'blue', 'purple', 'black', 'pink']
    for n, group in enumerate(groups):
        ...
        plt.scatter(label[:, 0], label[:, 1], c=color[n], marker='.', label='%s' % n)
    plt.scatter(center[:, 0], center[:, 1], c='red', marker='.', label='c')
    plt.xlabel('width')
    plt.ylabel('length')
    plt.legend(loc=2)
    plt.show()
