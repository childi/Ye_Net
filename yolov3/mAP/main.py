...
ground_truth_files_list = glob.glob('ground-truth_xy/*.txt')
...
for txt_file in ground_truth_files_list:
  ...
  if not os.path.exists('predicted_xy/' + file_id + ".txt"):
  ...
  for line in lines_list:
    try:
      if "difficult" in line:
          ...
          class_name, left, top, right, bottom, z = line.split()
          # if class_name == 'Car':
          #   nn = nn + 1
    except ValueError:
...
predicted_files_list = glob.glob('predicted_xy/*.txt')
...
for class_index, class_name in enumerate(gt_classes):
  ...
  for txt_file in predicted_files_list:
    ...
    file_id = os.path.basename(os.path.normpath(file_id))
    if class_index == 0:
      if not os.path.exists('ground-truth_xy/' + file_id + ".txt"):
    ...
    for line in lines:
      try:
        tmp_class_name, confidence, left, top, right, bottom, z = line.split()
      except ValueError:
        error_msg = "Error: File " + txt_file + " in the wrong format.\n"
...
with open(results_files_path + "/results.txt", 'a') as results_file:
  ...
  for class_name in sorted(pred_classes):
    ...
    recall = class_name + ": " + "recall= {0:.2f}%".format(recall * 100)
    print(recall)
