...
def _get_off_ang_loss(model, offsets, offsets_gt,
                      ...
                      cls_softmax, cls_gt,
                      mb_avod_bev_boxes, mb_bev_anchor_boxes_gt):
    ...
    # anchorwise_bevgiou_loss = 1 - bbox_giou(
    #     mb_avod_bev_boxes, mb_bev_anchor_boxes_gt)
    ...
    # pos_giou_loss = tf.reduce_sum(tf.boolean_mask(
    #     anchorwise_bevgiou_loss, positive_mask))

    # Combine regression losses
    combined_reg_loss = pos_localization_loss + pos_orientation_loss  # + pos_giou_loss
    ...
    with tf.variable_scope('reg_norm'):
        ...
        not_pos_num = tf.reduce_sum(tf.cast(tf.logical_not(positive_mask), tf.float32))
        ...
        # giou_loss_norm = tf.cond(
        #     pos_div_cond,
        #     lambda: pos_giou_loss / num_positives,
        #     lambda: tf.constant(0.0))
    ...
    if model._train_val_test == 'train':
        ...
        tf.summary.scalar('not_pos_num', not_pos_num)
        # tf.summary.scalar('giou_loss_norm', giou_loss_norm)
...
def _build_cls_off_ang_loss(model, prediction_dict):
    ...
    # + giou loss
    mb_bev_anchor_boxes_gt = prediction_dict[model.PRED_MB_A2BEV_GT]
    mb_avod_bev_boxes = prediction_dict[model.PRED_MB_PREDA2BEV]
    ...
    with tf.variable_scope('avod_losses'):
        ...
        with tf.variable_scope('regression'):
            final_reg_loss, offset_loss_norm, ang_loss_norm, num_positives = _get_off_ang_loss(
    ...
    losses_output['num_positives'] = num_positives
