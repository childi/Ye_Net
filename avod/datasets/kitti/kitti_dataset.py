...
class KittiDataset:
    ...
    def load_samples(self, indices):
        ...
        for sample_idx in indices:
            ...
            # sample_name = ['000500', '000500', '000500', '000500', '000500', '000500', '000500', '000500'][sample_idx]
            ...
            if self.has_labels:
                ...
                obj_labels = self.kitti_utils.filter_labels(obj_labels)  # , difficulty=2
                # 11train batch size=1, check if object_gt is empty
                # if (obj_labels is None) and self.train_val_test == 'train' \
                #         and (not self.train_on_all_samples):
                #     empty_sample_dict = {
                #         constants.KEY_SAMPLE_NAME: sample_name,
                #         constants.KEY_ANCHORS_INFO: 1
                #     }
                #     return [empty_sample_dict]
                ...
                label_boxes_2d = np.zeros((1, 6))
                label_classes = np.zeros(1)

            img_idx = int(sample_name)

            # Load image (BGR -> RGB)
            dense_image = np.load('/media/dataset/Kitti/object/training/dense_map_only_lidar/' + sample_name + '.npy')
            cv_bgr_image = cv2.imread(self.get_rgb_image_path(sample_name))
            rgb_image = cv_bgr_image[..., :: -1]
            image_shape = rgb_image.shape[0:2]
            # image_input = rgb_image
            image_input = np.concatenate((rgb_image, dense_image[:, :, np.newaxis]), axis=-1)
            ...
            point_cloud = self.kitti_utils.get_point_cloud(self.bev_source,
                                                           img_idx,
                                                           image_shape)
            # np.save('/media/personal_data/zhangye/bev_input/' + str(img_idx) + '.npy', point_cloud)
            ...
            if kitti_aug.AUG_FLIPPING in sample.augs:
                ...
                obj_labels = [kitti_aug.flip_label(obj, image_input)
            ...
            if obj_labels is not None:
                ...
                if len(label_boxes_3d) == 0:
                    ...
                    if self.train_on_all_samples:
                        ...
                        label_anchors = np.zeros((1, 6))
                        label_boxes_3d = np.zeros((1, 7))
                    label_classes = np.zeros(1)  # 0:not object; 1:car
                    label_boxes_2d = np.zeros((1, 6))
                else:
            ...
            # np.save('/media/personal_data/zhangye/bev_input/' + str(img_idx) + '.npy', bev_input)
            p2_I = np.linalg.pinv(stereo_calib_p2)
