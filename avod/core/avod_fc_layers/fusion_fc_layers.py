...
def _early_fusion_fc_layers(num_layers, layer_sizes,
    ...
    # fused_img = input_rois[1]
    # if len(input_rois) > 2:
    #     global_img = tf.tile(input_rois[2], [tf.shape(input_rois[0])[0], 1, 1, 1])
    #     mean_features = tf.concat([input_rois[1], global_img], axis=-1)
    #     with slim.arg_scope(
    #             [slim.conv2d],
    #             weights_regularizer=weights_regularizer):
    #         fused_img = slim.conv2d(mean_features,
    #                                 32,
    #                                 [1, 1],
    #                                 scope='concat_context')

    # fused_context = input_rois[1]
    # if len(input_rois) == 4 and input_weights[1] != tf.constant(0.0):
    #     global_img = tf.tile(input_rois[-1], [tf.shape(input_rois[0])[0], 1, 1, 1])
    #     instance_features = tf.concat([input_rois[2], global_img], axis=-1)
    #     with slim.arg_scope(
    #             [slim.conv2d],
    #             weights_regularizer=weights_regularizer):
    #         fused_img = slim.conv2d(instance_features,
    #                                 32,
    #                                 [1, 1],
    #                                 scope='concat_context')
    #         context_features = avod_fc_layer_utils.feature_fusion(fusion_method,
    #                                                               [fused_img, input_rois[1]],
    #                                                               tf.constant([1.0, 1.0]))
    #         # context_features = tf.concat([input_rois[1], fused_img], axis=-1)
    #         fused_context = slim.conv2d(context_features,
    #                                     32,
    #                                     [1, 1],
    #                                     scope='concat_context1')
    ...
    #                     weights_regularizer=weights_regularizer):
    #     fused_img = slim.conv2d(fused_features, 32, [1, 1], scope='concat_conv')
    #     fused_img = slim.conv2d(fused_img, 64, [3, 3], scope='concat_conv1')
    #     fused_img = slim.conv2d(fused_img, 32, [1, 1], scope='concat_conv2')
    #     fused_img = slim.conv2d(fused_img, 64, [3, 3], scope='concat_conv3')
    #     fused_img = slim.conv2d(fused_img, 32, [1, 1], scope='concat_conv4')
    ...
    with slim.arg_scope(
        ...
        # if len(input_rois) > 2:
        #     global_img = tf.tile(input_rois[2], [tf.shape(input_rois[0])[0], 1, 1, 1])
        #     mean_features = tf.concat([fused_features, global_img], axis=-1)
        #     fused_features = slim.conv2d(mean_features,
        #                                  32,
        #                                  [1, 1],
        #                                  scope='concat_context')
