...
class Evaluator:
    ...
    def __init__(self,
        ...
        if eval_mode == 'val':
            ...
            # self._total_loss = rpn_loss + avod_loss
        ...
        self._saver = tf.train.Saver(ema_obj.variables_to_restore())
    ...
    def run_checkpoint_once(self, checkpoint_to_restore):
        ...
        # saver_in.restore(self._sess, '/media/personal_data/zhangye/previous_results/yolov3_img&depth_z0604/yolov3_test_loss=8.9378.ckpt-1')
        ...
        global_step = evaluator_utils.strip_checkpoint_id(
        ...
        while current_epoch == self.model.dataset.epochs_completed:
            ...
            # feed_dict = {}
            # image_first_name = sample_names[i]
            # i = i+1
            # image_path = os.path.join('/media/dataset/Kitti/object/training/image_2', image_first_name + '.png')
            ...
            # image_data = utils.image_preporcess(image, [384, 3 * 384])
            # org_h, org_w, _ = image.shape
            # feed_dict[self.model.placeholders[self.model.PL_YOLO_INPUT]] = image_data
            # feed_dict[self.model.placeholders[self.model.PL_IMG_IDX]] = [int(image_first_name)]
            # feed_dict[self.model.placeholders[self.model.PL_IMG_SHAPE]] = [org_h, org_w]
            ...
            # for i in range(self.model._rpn_model.batch_size):
            # Get sample name from model
            sample_name = self.model.sample_info['sample_name']
            # sample_name = '%06d' % sample_name[i][0]  #
            ...
            # np.savetxt(rpn_file_path, proposals_and_scores, fmt='%s')
            if self.full_model:
    ...
    def run_latest_checkpoints(self):
        ...
        if ckpt_indices is not None:
            ...
            for ckpt_idx in ckpt_indices:
                ...
                # checkpoint_to_restore = '/media/personal_data/zhangye/outputs/pyramid_cars_with_aug_example/checkpoints' + \
                #                         '/pyramid_cars_with_aug_example-00%03d000' % ckpt_idx  #
                checkpoint_to_restore = '/media/personal_data/zhangye/outputs/' + self.model_config.checkpoint_name +\
                                        '/checkpoints' +\
                                        '/%s-00%06d' % (self.model_config.checkpoint_name, ckpt_idx * 6733)  #
            ...
            if number_of_evaluations >= num_checkpoints:
                ...
                for ckpt_idx in range(50, num_checkpoints+1):
                    ...
                    checkpoint_to_restore = '/media/personal_data/zhangye/outputs/' + self.model_config.checkpoint_name +\
                                            ...
                                            '/%s-00%06d' % (self.model_config.checkpoint_name, ckpt_idx * 6733)  #
                    # checkpoint_to_restore = '/media/personal_data/zhangye/outputs/pyramid_cars_with_aug_example/checkpoints' + \
                    #                         '/pyramid_cars_with_aug_example-00000001'  #
                    ckpt_id = evaluator_utils.strip_checkpoint_id(
                        checkpoint_to_restore)
    ...
    def _update_avod_box_cls_loc_orient_losses(self,
        ...
        if (avod_localization_loss > 0.0) and \
            ...
            eval_avod_losses.update({KEY_NUM_VALID_REG_SAMPLES:
                                     num_valid_regression_samples})
        print('NUM_VALID_REG_SAMPLES', eval_avod_losses[KEY_NUM_VALID_REG_SAMPLES])
    ...
    def get_rpn_proposals_and_scores(self, predictions):
        ...
        #                                         softmax_scores))
        prps = predictions[RpnModel.PRED_PRPS]
        ...
        for bbox in prps:
            ...
            alpha = str(bbox[6])
            bbox_mess.append([class_name, score, xmin, ymin, xmax, ymax, alpha])

        return np.array(bbox_mess)
