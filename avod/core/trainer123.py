...
def train(model, train_config):
    ...
    # Setup loss
    ##############################
    losses_dict, rpn_loss, avod_loss = model.loss(prediction_dict)
    ...
    trainable_conv_layer = ['conv%d' % (52 + i) for i in range(17)]  # 'conv52'到'conv68'
    trainable_all_name = ['conv_sobj_branch', 'conv_mobj_branch', 'conv_lobj_branch', 'darknet',
                          'conv_lbbox', 'conv_mbbox', 'conv_sbbox']
    trainable_all_name.extend(trainable_conv_layer)
    with tf.name_scope("define_first_stage_train"):
        ...
        first_stage_optimizer = tf.train.AdamOptimizer(learn_rate).minimize(
            rpn_loss, var_list=first_stage_trainable_var_list)
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            with tf.control_dependencies([first_stage_optimizer, global_step_update]):
                with tf.control_dependencies([moving_ave]):
                    train_op_with_yolov3_variables = tf.no_op()

    with tf.name_scope("define_second_stage_train"):
        second_stage_trainable_var_list = [val for val in var if str(val.name).split('/')[0] not in trainable_all_name]
        second_stage_optimizer = tf.train.AdamOptimizer(learn_rate).minimize(
            avod_loss, var_list=second_stage_trainable_var_list)
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            with tf.control_dependencies([second_stage_optimizer, global_step_update]):
                with tf.control_dependencies([moving_ave]):
                    train_op_with_avod_variables = tf.no_op()

    # with tf.name_scope("define_third_stage_train"):
    #     third_stage_trainable_var_list = tf.trainable_variables()
    #     third_stage_optimizer = tf.train.AdamOptimizer(learn_rate).minimize(
    #         avod_loss, var_list=third_stage_trainable_var_list)
    #
    #     with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    #         with tf.control_dependencies([third_stage_optimizer, global_step_update]):
    #             with tf.control_dependencies([moving_ave]):
    ...
    tf.summary.scalar("training_loss", avod_loss)
    ...
    if not train_config.overwrite_checkpoints:
        ...
        if len(saver.last_checkpoints) > 0:
            ...
            all_name = ['conv_sobj_branch', 'conv_mobj_branch', 'conv_lobj_branch',
                        'darknet', 'conv_lbbox', 'conv_mbbox', 'conv_sbbox']  #
            ...
            saver_in.restore(sess,
                             '/media/personal_data/zhangye/previous_results/yolov3_img&depth_z0604/yolov3_test_loss=8.9378.ckpt-1')
    ...
    print('Step {} / {}, Checkpoint saved to {}-{:08d}'.format(
        global_step, max_iterations, checkpoint_path, global_step))
    ...
    for step in range(global_step, max_iterations + 1):
        if global_step // checkpoint_interval < 30:
            train_op = train_op_with_yolov3_variables
            op_loss = rpn_loss
        elif global_step // checkpoint_interval < 80:
            train_op = train_op_with_avod_variables
            op_loss = avod_loss
        # else:
        ...
        if step % summary_interval == 0:
            ...
            train_op_loss, _, summary_out = sess.run(
                [op_loss, train_op, summary_merged], feed_dict=feed_dict)
            ...
            sess.run([op_loss, train_op], feed_dict)

        # _, test_loss = sess.run([prediction_dict, total_loss], feed_dict_test)

        # if step < 10:
        #     print('Step {}, train_loss {:0.3f}'.format(step, train_op_loss))
        # else:
        #     # if step % 3712 == 0:
