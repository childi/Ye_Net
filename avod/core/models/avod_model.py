...
class AvodModel(model.DetectionModel):
    ...
    PRED_MB_A2BEV_GT = 'avod_mb_anchors2bev'
    ...
    PRED_MB_PREDA2BEV = 'avod_mb_pred2anchors2bev'
    ...
    def build(self):
        ...
        # self._is_training = rpn_model._is_training

        # top_anchors = prediction_dict[RpnModel.PRED_TOP_ANCHORS]
        top_anchors = rpn_model.placeholders[RpnModel.PL_TOP_ANCHORS]
        avod_projection_in = top_anchors

        # top_anchors_a = rpn_model.placeholders[RpnModel.PL_TOP_ANCHORS]
        # avod_projection_in = top_anchors_a
        # gen_3d = rpn_model.placeholders[RpnModel.PL_GEN_BOX]
        # top_anchors = gen_3d[:, 0:6]

        # self.batch_size = rpn_model.batch_size
        # prediction_dicts = dict()
        # for i in range(self.batch_size):
        #     top_anchors = batch_top_anchors['the %d batch top anchors' % i]
        ground_plane = rpn_model.placeholders[RpnModel.PL_GROUND_PLANE]  # [i]

        class_labels = rpn_model.placeholders[RpnModel.PL_LABEL_CLASSES]  # + str(i)
            ...
            with tf.variable_scope('bev'):
                ...
                bev_proposal_boxes_tf_order = \
                    anchor_projector.reorder_projected_boxes(
                        bev_proposal_boxes)  # -->mb mask
                bev_proposal_boxes_norm_tf_order = \
                    anchor_projector.reorder_projected_boxes(
                        bev_proposal_boxes_norm)  # -->FE

            with tf.variable_scope('img'):
                image_shape = tf.cast(
                    rpn_model.placeholders[RpnModel.PL_IMG_SHAPE],
                    tf.float32)  #[i]
        ...
        bev_feature_maps = rpn_model.bev_feature_maps  #[i][tf.newaxis, :, :, :]
        img_feature_maps = rpn_model.img_feature_maps  #[i][tf.newaxis, :, :, :]
        # img_global_maps = rpn_model.img_global_fe
        ...
        with tf.variable_scope('avod_roi_pooling'):
            ...
            img_rois = tf.image.crop_and_resize(
                ...
                name='img_rois')
            # # Do ROI Pooling on image
            # img_global_rois = tf.image.crop_and_resize(
            #     img_global_maps,
            #     img_proposal_boxes_norm_tf_order,
            #     tf_box_indices,
            #     self._proposal_roi_crop_size,
            #     name='img_rois')
            # # Do Pooling on global image
            # img_global_ = tf.image.crop_and_resize(
            #     img_global_maps,
        ...
        fc_output_layers = \
            avod_fc_layers_builder.build(
                ...
                is_training=self._is_training)  # , img_global_rois, img_global_  # , img_mask, img_mask
        ...
        with tf.variable_scope('softmax'):
            # all_cls_logits = tf.Print(all_cls_logits, [all_cls_logits], 'all_cls_logits', summarize=100)
            all_cls_softmax = tf.nn.softmax(
                all_cls_logits)
            # all_cls_softmax = tf.Print(all_cls_softmax, [all_cls_softmax], 'all_cls_softmax', summarize=100)

        ######################################################
        # Subsample mini_batch for the loss function
        ######################################################
        # Get the ground truth tensors
        anchors_gt = rpn_model.placeholders[RpnModel.PL_LABEL_ANCHORS]  # + str(i)
        if self._box_rep in ['box_3d', 'box_4ca']:
            boxes_3d_gt = rpn_model.placeholders[RpnModel.PL_LABEL_BOXES_3D]  # + str(i)
        ...
        with tf.variable_scope('bev_avod_rois'):
            ...
            bev_input_rois = tf.image.crop_and_resize(
                self._rpn_model._bev_input_batches,
                mb_bev_anchors_norm,
                mb_bev_box_indices,
                (32, 32))  #[i][tf.newaxis, :, :, :]
        ...
        with tf.variable_scope('img_avod_rois'):
            ...
            img_input_rois = tf.image.crop_and_resize(
                self._rpn_model._yolov3_input_batches,
                mb_img_anchors_norm,
                mb_img_box_indices,
                (32, 32))  #[i][tf.newaxis, :, :, :]
        ...
        with tf.variable_scope('avod_nms'):
            ...
            nms_indices = tf.image.non_max_suppression(
                ...
                iou_threshold=self._nms_iou_threshold)  # [:-1]
        ...
        # + giou loss
        mb_bev_anchor_boxes_gt = tf.boolean_mask(bev_anchor_boxes_gt, mb_mask)
        mb_avod_bev_boxes = tf.boolean_mask(avod_bev_boxes, mb_mask)
        ...
        if self._train_val_test in ['train', 'val']:
            ...
            prediction_dict[self.PRED_MB_PREDA2BEV] = mb_avod_bev_boxes
            ...
            prediction_dict[self.PRED_MB_A2BEV_GT] = mb_bev_anchor_boxes_gt
        ...
        # prediction_dict[self.PRED_ALL_IOUS] = all_ious
            ...
            # prediction_dicts[str(i)] = prediction_dict
    ...
    # def create_feed_dict_test(self, dataset_test=None):
    #     feed_dict = self._rpn_model.create_feed_dict_test(dataset_test)
    #     return feed_dict
    ...
    def loss(self, prediction_dict):
        ...
        num_positives = losses_output.get('num_positives')
        ...
        # avod_loss = 0
        # offset_loss_norm = 0
        # ang_loss_loss_norm = 0
        # num_positives = 0
        # for i in range(self.batch_size):
        #     losses_output = avod_loss_builder.build(self, prediction_dict[str(i)])
        #
        #     classification_loss += \
        #         losses_output[avod_loss_builder.KEY_CLASSIFICATION_LOSS]
        #
