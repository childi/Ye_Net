...
class AvodModel(model.DetectionModel):
    ...
    def build(self):
        ...
        batch_top_anchors = prediction_dict[RpnModel.PRED_TOP_ANCHORS]
        ...
        for i in range(self.batch_size):
            ...
            with tf.variable_scope('avod_projection'):
                ...
                with tf.variable_scope('img'):
                    image_shape = tf.cast(
                        rpn_model.placeholders[RpnModel.PL_IMG_SHAPE][i],
                        tf.float32)  #
            ...
            with tf.variable_scope('bev_avod_rois'):
                ...
                bev_input_rois = tf.image.crop_and_resize(
                    self._rpn_model._bev_input_batches[i][tf.newaxis, :, :, :],
                    mb_bev_anchors_norm,
                    mb_bev_box_indices,
                    (32, 32))  #
            ...
            with tf.variable_scope('img_avod_rois'):
                ...
                img_input_rois = tf.image.crop_and_resize(
                    self._rpn_model._yolov3_input_batches[i][tf.newaxis, :, :, :],
                    mb_img_anchors_norm,
                    mb_img_box_indices,
                    (32, 32))  #
