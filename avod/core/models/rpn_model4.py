...
from avod.core.box_3d_encoder import box_3d_to_anchor
...
class RpnModel(model.DetectionModel):
    ...
    def _set_up_input_pls(self):
        ...
        with tf.variable_scope('pl_anchors'):
            ...
            with tf.variable_scope('sample_info'):
                ...
                self._add_placeholder(tf.float32, [None, 2], self.PL_IMG_SHAPE)

        # Combine config data
        # bev_dims = np.append(self._bev_pixel_size, self._bev_depth)
        bev_dims = [None, None, None, self._bev_depth]
        ...
        with tf.variable_scope('yolov3_input'):
            ...
            true_sbboxes = self._add_placeholder(tf.float32, [None, self.max_bbox_per_scale, 4],
                                                 self.PL_SBBOXES)
            # self.true_sbboxes = tf.expand_dims(true_sbboxes, axis=0)
            self.true_sbboxes = true_sbboxes
            true_mbboxes = self._add_placeholder(tf.float32, [None, self.max_bbox_per_scale, 4],
                                                 self.PL_MBBOXES)
            # self.true_mbboxes = tf.expand_dims(true_mbboxes, axis=0)
            self.true_mbboxes = true_mbboxes
            true_lbboxes = self._add_placeholder(tf.float32, [None, self.max_bbox_per_scale, 4],
                                                 self.PL_LBBOXES)
    ...
    def _set_up_feature_extractors(self):
        ...
        self.model_img = self.yolo_model(self._yolov3_input_batches, self._is_training, avod=True)
    ...
    def build(self):
        ...
        def process(pred_bbox, img_shape, stereo_calib_p2, sample_name):
            ...
            bboxes = utils.postprocess_boxes_inavod(pred_bbox, img_shape, self.train_input_size, self.score_threshold)
            bboxes = utils.nms(bboxes, self.iou_threshold)
            # print('bboxes', bboxes)
            proposal = [v[0:4] for v in bboxes]
            classid = [v[5] for v in bboxes]
            alpha2d = [v[6] for v in bboxes]
            ...
            box_obj, box3 = ppb.GS3D().proposal2bbox(proposal, classid, p2, sample_name,
                                                     filter_class=avod_class)  # , alpha=alpha2d, aug=['multiple']
            # print('box3', box3)
            if self.show_bbox_in_bev:
                box_gt = obj_utils.read_labels('/media/dataset/Kitti/object/training/label_2/', int(sample_name))
                ppb.GS3D().draw3dbox_xz(sample_name, box_obj, box_gt)

            top_anchor = box_3d_to_anchor(box3)
            # print('top_anchor', top_anchor)
        ...
        # top_anchors = tf.reshape(tf.py_func(process, [pred_bbox, img_shape, stereo_calib_p2, sample_name[0]],
        #                                     tf.float32), [-1, 6])
        # 22train batch size>1:
        top_anchors = dict()
        for i in range(self.batch_size):
    ...
    def create_feed_dict(self, sample_index=None):
        ...
        # box_path = os.path.join('/media/dataset/Kitti/object/training/label_2', str(sample_name) + '.txt')
        ...
        # avod_class = self.dataset.classes
        # _, box3 = ppb.GS3D().proposal2bbox(proposal, classes, stereo_calib_p2, filter_class=avod_class, aug=True)
    ...
    def loss(self, prediction_dict):
        ...
        giou_loss, conf_loss, prob_loss, alpha_loss = self.model_img.compute_loss(
            self.label_sbbox, self.label_mbbox, self.label_lbbox, self.true_sbboxes, self.true_mbboxes, self.true_lbboxes)
        total_loss = giou_loss + conf_loss + prob_loss + alpha_loss
