...
def object_label_to_box_2d(obj_label):
    ...
    Returns:
        anchor: 2D box in box_2d format [xi, yi, xa, ya, class_id, alpha]
    """

    cla = ['Car', 'Pedestrian', 'Cyclist']
    ...
    box_2d[4] = cla.index(obj_label.type)
    box_2d[5] = obj_label.t[2]  # obj_label.alpha
...
def box_3d_to_avod_projected_in(boxes_3d, ortho_rotate=False):
    """ Converts a box_3d [x, y, z, l, w, h, ry]
    into anchor form [x, y, z, dim_x, dim_y, dim_z] use for avod_projected_in

    anchor :[x, y, z, dim_x, dim_y, dim_z], dim_x, dim_y, dim_z is preset value, ignore ry
    ...
    # dim_x, dim_y, dim_z
    anchors[:, [3]] = np.ones((num_anchors, 1)) * 4
    anchors[:, [4]] = np.ones((num_anchors, 1)) * 1.5
    anchors[:, [5]] = np.ones((num_anchors, 1)) * 4
