...
def save_predictions_in_kitti_format(model,
    ...
    final_predictions_root_dir = '/media/personal_data/zhangye/outputs/' + checkpoint_name + '/predictions' + \
    ...
    for sample_idx in range(num_samples):
        ...
        all_predictions = np.reshape(all_predictions, [-1, 9])
...
def print_inference_time_statistics(total_feed_dict_time,
    ...
    # total_rpn_model_time = np.asarray(total_rpn_model_time)
    # print('rpn time:')
    # print('Min: ', np.round(np.min(total_rpn_model_time), 5))
    # print('Max: ', np.round(np.max(total_rpn_model_time), 5))
    # print('Mean: ', np.round(np.mean(total_rpn_model_time), 5))
    # print('Median: ', np.round(np.median(total_rpn_model_time), 5))
