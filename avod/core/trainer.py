...
def train(model, train_config):
    ...
    with tf.name_scope("define_first_stage_train"):
        ...
        trainable_all_name = ['conv_sobj_branch', 'conv_mobj_branch', 'conv_lobj_branch', 'darknet',
                              'conv_lbbox', 'conv_mbbox', 'conv_sbbox', 'upsample0', 'upsample1']
        trainable_all_name.extend(trainable_conv_layer)
        first_stage_trainable_var_list = [val for val in var if str(val.name).split('/')[0] not in trainable_all_name]
    ...
    if not train_config.overwrite_checkpoints:
        ...
        if len(saver.last_checkpoints) > 0:
            ...
            all_name = ['conv_sobj_branch', 'conv_mobj_branch', 'conv_lobj_branch',
                        'darknet', 'conv_lbbox', 'conv_mbbox', 'conv_sbbox', 'upsample0', 'upsample1']  #
            ...
            saver_in.restore(sess,
                             '/media/personal_data/zhangye/previous_results/yolov3_img&depth_zz1030/2dgt/yolov3_test_loss=17.4437.ckpt-76')
            # /media/personal_data/zhangye/previous_results/yolov3_img&depth_zz1030/3dgt_2d/yolov3_test_loss=16.8527.ckpt-93
            #
            # /media/personal_data/zhangye/yolov3_ckpt0826/yolov3_test_loss=35.8165.ckpt-61
